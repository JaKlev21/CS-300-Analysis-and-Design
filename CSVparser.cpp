...
namespace csv {
    ...
    : _type(type), _sep(sep)
  {
      ...
      if (type == eFILE)
      {
        ...
        if (ifile.is_open())
        {
            ...
            //[Jonathan Kleven] Commented out parseHeader
            //parseHeader();
            parseContent();
        }
        else
  ...
  void Parser::parseContent(void)
  {
     ...
     //[Jonathan Kleven] Commented out increment "it" to not skip header row
     //it++; // skip header
     ...
     for (; it != _originalFile.end(); it++)
     {
         ...
         // if value(s) missing
         //[Jonathan Kleven] if data has less than 2 parameters
         if (row->size() < 2) {// Changed from "!= _header.size()
